...
pub fn get_todo_file_as_json(call: &EvaluatedCall) -> Result<JsonValue, LabeledError> {
    let file_path = match call.get_flag::<PathBuf>("file")? {
        Some(path) => Ok(path),
        None => dirs::home_dir()
            .ok_or_else(|| {
    ...
    Ok(TodoFile::new(&file_path.to_string_lossy()).as_json())
}
